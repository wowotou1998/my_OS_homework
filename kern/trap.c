...
trap_init(void)
{
	...
	/*
	* You shouldn't call a TRAPHANDLER function from C, 
	* but you may need to _declare_ one in C   (for instance, to get a function pointer during IDT setup).
	* You can declare the function with void NAME();
	* where NAME is the argument passed to TRAPHANDLER.
	*/
	// LAB 3: Your code here.
	// these function declares (you must know that the declares just declares and they don't have the function body!!!) 
	// these function's content are all defined  in the file "trapentry.S"
	void t_divide();
	void t_debug();
	void t_nmi();
	void t_brkpt();
	...
	// this interrupt descripter DPL is 3
	// 如果设置其DPL为0，则会产生GPF，因为用户程序跳转执行内核态程序。
	//如果我们想要当前执行的程序能够跳转到这个描述符所指向的程序哪里继续执行的话，
	//有个要求，就是要求当前运行程序的CPL，RPL的最大值需要小于等于DPL，
	...
	// IRQ  SETGATE(gate, istrap, selector, offset, DPL)		
...
trap_dispatch(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	// Only if there is one trap function  can match the trapno,we should must run and then just return ! not break the loop
	switch(tf->tf_trapno) {
		case T_PGFLT: 
			page_fault_handler(tf);
			return ;
		case T_BRKPT:
			cprintf("the env %d  breakpoint() \n",curenv->env_id);
			//monitor(tf);
			return ;
	...
	cprintf("Unexpected trap happened\n");
	print_trapframe(tf);
	if (tf->tf_cs == GD_KT)
