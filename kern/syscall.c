...
#define QueueSize 1025 // 队伍容量
#define DataType int // 队伍容量
...
void sys_semaphore_p(void){
	s1->current_value--;
	cprintf("you(%d) are about to get semaphore,curren_value is %d\n",curenv->env_id,s1->current_value);
	if((s1->current_value)<0){
		cprintf("you(%d) have been blocked\n",curenv->env_id);
		curenv->env_status = ENV_NOT_RUNNABLE;
		queue_push(curenv->env_id);
		sched_yield();
	}
...
void sys_semaphore_v(void){
	s1->current_value++;
	if(s1->current_value < s1->original_value){
		cprintf("you(%d) are about to return semaphore,curren_value is %d\n",curenv->env_id,s1->current_value);
		temp_id = queue_pop();
	}

	envs[temp_id].env_status = ENV_RUNNABLE;
	cprintf("you(%d) will be running\n",envs[temp_id].env_id);
	env_run(&envs[temp_id]);
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_semaphore_p:
		sys_semaphore_p();
	
	case SYS_semaphore_v:
		sys_semaphore_v();
